...
- (void)setWidthsWithArray:(CGPDFArrayRef)widthsArray
{
    ...
    while (idx < length)
    {
		...
		if (CGPDFObjectGetType(integerOrArray) == kCGPDFObjectTypeInteger)
		{
			...
			for (NSUInteger index = 0; index < widthsCount ; index++)
			{
				CGPDFInteger width;
				if (CGPDFArrayGetInteger(characterWidths, index, &width))
				{
					NSNumber *key = [NSNumber numberWithInteger:firstCharacter+index];
					NSNumber *val = [NSNumber numberWithInteger:width];
...
- (void)setWidthsFrom:(CGPDFInteger)cid to:(CGPDFInteger)maxCid width:(CGPDFInteger)width
{
    while (cid <= maxCid)
    {
        [self.widths setObject:[NSNumber numberWithInteger:width] forKey:[NSNumber numberWithInteger:cid++]];
    }
}
...
- (void)setWidthsWithBase:(CGPDFInteger)base array:(CGPDFArrayRef)array
{
    ...
    for (int index = 0; index < count ; index++)
    {
        ...
        {
            [self.widths setObject:[NSNumber numberWithInteger:width] forKey:[NSNumber numberWithInteger:base+index]];
        }
    }
}
